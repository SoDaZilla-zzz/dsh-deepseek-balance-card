...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		function injectStyles() {
			...
			style.textContent = `
				...
				.dsbc-card-content {
					...
					box-sizing: border-box;
					width: 100%;
					height: var(--dsbc-card-height, auto);
					border-radius: 20px;
		...
		function DeepSeekBalanceCard() {
			...
			const dailyValues = dailyData.map((d) => chartMetric === "cost" ? (d.cost || 0) : (d.totalTokens || 0));
			...
			const glassStyle = {
				...
				"--dsbc-card-height": glass.cardHeight > 0 ? `${glass.cardHeight}px` : "auto"
			};
			const cardStyle = {
				...glassStyle,
														...
														children: [
															...
															jsx("div", {
																...
																children: dailyData.map((d, i) => {
																	const value = chartMetric === "cost" ? (d.cost || 0) : (d.totalTokens || 0);
																	const pct = Math.max(2, Math.round(value / dailyMax * 100));
																	const label = chartMetric === "cost" ? formatCost(d.cost, currency) : formatTokens(d.totalTokens);
								...
								children: [
									...
									jsxs("div", {
										...
										children: [
											...
											jsx("div", {
												className: "dsbc-settings-info",
												children: "高度为 0 时自动适应内容；设置后内容超出会出现滚动。"
											})
										]
									}),
									jsxs("div", {

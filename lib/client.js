...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		const DEFAULT_GLASS = { alpha: 14, blur: 4, saturate: 140, highlight: 0.35, shine: 0.14 };
				...
				if (raw) {
					...
					if (g !== null && typeof g === "object") {
						...
						for (const key of Object.keys(DEFAULT_GLASS)) {
							if (typeof g[key] === "number" && Number.isFinite(g[key])) {
								out[key] = g[key];
							}
						}
				...
				@supports ((backdrop-filter: blur(1px)) or (-webkit-backdrop-filter: blur(1px))) {
					.dsbc-card {
						background: linear-gradient(
							135deg,
							color-mix(in srgb, var(--dsw-alias-bg-overlay, #ffffff) var(--dsbc-alpha, 14%), transparent),
							color-mix(in srgb, var(--dsw-alias-bg-overlay, #ffffff) var(--dsbc-alpha2, 7%), transparent)
						);
						...
						border-color: color-mix(in srgb, var(--dsw-alias-border-l2, rgba(255, 255, 255, 0.6)) 35%, transparent);
					}
				}
